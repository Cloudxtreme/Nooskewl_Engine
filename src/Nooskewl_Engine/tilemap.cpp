...
float Tilemap::get_z(int layer, int x, int y)
{
	...
	for (size_t i = 0; i < l.groups.size(); i++) {
		...
		if (x >= g->position.x && y >= g->position.y && x < (g->position.x+g->size.w) && y < (g->position.y+g->size.h)) {
			...
			return -(1.0f - 0.01f * ((float)((g->position.y + g->size.h - 1) * noo.tile_size) / (float)(size.h * noo.tile_size)));
		}
	}
	return -(1.0f - 0.01f * ((float)(y * noo.tile_size) / (float)(size.h * noo.tile_size)));
}
